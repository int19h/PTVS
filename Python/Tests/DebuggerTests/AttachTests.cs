...
namespace DebuggerTests {
    ...
    public abstract class AttachTests : BaseDebuggerTests {
        ...
        private static async Task CancelAfter(int milliseconds, string message) {
            ...
            try {
                using (var dumpWriter = new MiniDumpWriter(p)) {
                    ...
                    for (int i = 0; i < 10; i++) {
                        ...
                        await Task.WhenAny(attached.Task, CancelAfter(30000, "Failed to attach within 30s")).Unwrap();
                        await proc.DetachAsync(TimeoutToken());
                        await Task.WhenAny(detached.Task, CancelAfter(30000, "Failed to detach within 30s")).Unwrap();
                    }
            ...
            try {
                using (var dumpWriter = new MiniDumpWriter(p)) {
                    ...
                    try {
                        ...
                        await Task.WhenAny(attached.Task, CancelAfter(20000, "Failed to attach within 20s")).Unwrap();
                        ...
                        await Task.WhenAny(bpHit.Task, CancelAfter(20000, "Failed to hit breakpoint within 20s")).Unwrap();
                    } finally {
                        await DetachProcessAsync(proc);
                    }
        ...
        private async Task AttachTestTimeoutAsync(string hostCode) {
            ...
            try {
                using (var dumpWriter = new MiniDumpWriter(p)) {
                    ...
                    await Task.WhenAny(attachTask, attachStarted.Task, CancelAfter(10000, "Failed to start attaching within 10s")).Unwrap();
                    Assert.IsFalse(isAttached, "Should not have attached yet"); // we should be blocked

                    handle.Set();   // let the code start running

                    await Task.WhenAny(attachTask, attachDone.Task, CancelAfter(10000, "Failed to attach within 10s"));
                    dumpWriter.Cancel();
                }
            } finally {
